...
async def identifying_object_stream(image):
    ...
    if len(capacitor_detected[0].boxes) > 0:
        yield json.dumps({"status": "detected", "type": "capacitor"}).encode('utf-8')
        for i, result in enumerate(capacitor_detected[0].boxes):
            ...
            if len(terminal_detected[0].boxes) > 0:
                ...
                yield json.dumps({"status": "detected", "type": "terminal", "detected_objects": detected_objects}).encode('utf-8')
            else:
                yield json.dumps({"status": "notDetected", "type": "terminal"}).encode('utf-8')

    else:
        yield json.dumps({"status": "notDetected", "type": "capacitor"}).encode('utf-8')
